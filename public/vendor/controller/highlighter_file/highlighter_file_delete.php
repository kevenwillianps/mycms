...
use \vendor\model\HighlighterFile;

/** Instânciamento das classes importadas **/
$main = new Main();
$highlighterFile = new HighlighterFile();
...
try {
    ...
    if ($main->verifySession()){
        ...
        $highlighter_file_id = isset($inputs['inputs']['highlighter_file_id']) ? (int)$main->antiInjection($inputs['inputs']['highlighter_file_id']) : 0;
        ...
        if (count($message) > 0) {
            ...
            $row = $highlighterFile->get($highlighter_file_id);

            /** Verifico se existe o registro **/
            if (isset($row)) {

                /** Verfico se o registro é válido **/
                if ($row->highlighter_file_id > 0) {

                    /** Executo o método de exclusão **/
                    if ($highlighterFile->delete($row->highlighter_file_id)) {

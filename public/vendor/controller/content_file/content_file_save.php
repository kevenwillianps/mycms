...
try {
    ...
    if ($main->verifySession()){
        ...
        $highlighter_file_id = isset($inputs['inputs']['highlighter_file_id']) ? (int)$main->antiInjection($inputs['inputs']['highlighter_file_id']) : 1;
        $position            = isset($inputs['inputs']['position'])            ? (int)$main->antiInjection($inputs['inputs']['position'])            : 0;
        ...
        if ($highlighter_file_id < 0) {
            array_push($message, '$highlighter_file_id - O seguinte campo deve ser preenchido/selecionado');
        }
            ...
            if (is_dir($path)) {
                if ($pointer == ($length - 1)) {
                    ...
                    if (is_file($path . '/' . $name)) {
                        ...
                        if ($file->handling($path, $name)) {

                            $contentFile->save($content_file_id, $content_id, $user_id, $situation_id, $highlighter_file_id, $position, $name, $description, $base64, $file_type, $path, $date_register, $date_update);
                ...
                mkdir($path . '/resize', 0777, true);
                mkdir($path . '/thumbnail', 0777, true);
                mkdir($path . '/filter_gray_scale', 0777, true);
                ...
                if (is_dir($path)) {
                    if ($pointer == ($length - 1)) {
                        ...
                        if (is_file($path . '/' . $name)) {
                            ...
                            /** Realizo o tratamento de imagens **/
                            if ($file->handling($path, $name)) {

                                /** Salvo o Registro das imagens **/
                                $contentFile->save($content_file_id, $content_id, $user_id, $situation_id, $highlighter_file_id, $position, $name, $description, $base64, $file_type, $path, $date_register, $date_update);
